...
burgerMenu.onmouseleave = function() {	if (burgMOpn==true) {burgMenSw();}	}
...
//корректировка сожержимого элементов unt в разделе tutors============>>>
if (window.innerWidth < 700) {
	let tutors = document.querySelectorAll(".tutors");
	for (let i = 0; i < tutors.length; i++) {
		let unt = tutors[i].querySelectorAll(".unt");
		for (let i = 0; i < unt.length; i++) {
			let educ = unt[i].querySelector(".educ");
			let ul = unt[i].querySelector("ul");
			let askCall = unt[i].querySelector(".askCall");
			...
			cont2.append(educ);
			cont2.append(ul);
			cont2.append(askCall);
			askCall.style.marginLeft = "auto";
			askCall.style.marginRight = "auto";		}	}	}
//============>>>корректировка сожержимого элементов unt в разделе tutors
